...
public class MySharedPreference {
	...
	//调用上面的 share(Context context) 方法 将数据存入，并标识为 "name"
	//使用 commit() 方法保存会给出反应（保存成功或失败）
	public static boolean setId(Integer id, Context context){
		SharedPreferences.Editor e = share(context).edit();
		e.putInt("id",id);
		Boolean bool = e.commit();
		return bool;
	...
	public static String getEmail(Context context){
		return share(context).getString("email",null);
	}
	...
	public static Integer getAge(Context context){
		return share(context).getInt("age",0);
	}
	public static void setAge(Integer age, Context context){
		SharedPreferences.Editor e = share(context).edit();
		e.putInt("age",age);
		e.apply();
	}
